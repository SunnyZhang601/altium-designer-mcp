...
| Via | simple TH, two pad/hole sizes; ✅ mask-expansion cache state + 4 mil template expansion (`samples_pcblib_via_mask_state_is_altium_factory_default` — an Altium via carries byte @66 = 0, `eCacheInvalid`) | thermal-relief, power-plane, paste-mask expansion, GUID; 🔒 net index; 🚫 tenting flags (`IsTenting_Top`/`_Bottom` author fine but AD24 does not persist them on a library via — the saved flag word is empty and the reader decodes those bits correctly elsewhere) |
