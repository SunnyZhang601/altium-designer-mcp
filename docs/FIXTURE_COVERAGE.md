...
| Text | stroke text, Win-1252 chars, vertical (90°); ✅ TrueType `font_name`='Arial' + bold + italic + mirror (`TEXT_STYLE`); ✅ kind=BarCode (`TEXT_SPECIAL` 'BC128'); ✅ inverted (knockout) text + inverted-rect descriptor (`TEXT_SPECIAL` 'INV': `is_inverted`, `use_inverted_rectangle`, `inverted_border`=10 mil, auto-computed rect width/height exact-asserted) | justification, stroke_font variants, ✅ barcode sizing block (`TEXT_SPECIAL` 'BC2': full width/height, X/Y margins, symbology and UTF-16LE font name, offsets derived by diffing two barcodes with differing sizing); ❌ barcode MinWidth (Altium computes it) / Inverted / ShowText / RenderMode |
