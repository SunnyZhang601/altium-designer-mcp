...
| Text | stroke text, Win-1252 chars, vertical (90°); ✅ TrueType `font_name`='Arial' + bold + italic + mirror (`TEXT_STYLE`); ✅ kind=BarCode (`TEXT_SPECIAL` 'BC128'); ✅ inverted (knockout) text + inverted-rect descriptor (`TEXT_SPECIAL` 'INV': `is_inverted`, `use_inverted_rectangle`, `inverted_border`=10 mil, auto-computed rect width/height exact-asserted) | justification, stroke_font variants, 🚫 barcode sizing block — AD24 exposes no setter for any of its ten keys and the golden's barcode and non-barcode records carry identical template values at the candidate offsets, so the mapping cannot be validated by diffing, char_set, union_index, component index, flags; 🔒 net index |
